function [T21,err90,err95,err99] = multi_causality_est_all_new(xx, np,st)
% recoded by Rong Yineng (yinengrong@foxmail.com)
% function [T21, err90,err95,err99] = multi_causality_est_all(xx, np,st)
...
%nm = Number of observations / M = number of time series
[nm, M] = size(xx);

%Calculate the derived series Xj using the Euler forward differencing
%approximation of dXj/dt
for k = 1 : M
...
for kk=1 : M
    ...
    dC(k,kk) = sum((x(:,k) - mean(x(:,k))) .* (dx(:,kk) - mean(dx(:,kk)))); % Equation (13)
    end
end
dC = dC / (N-1);
